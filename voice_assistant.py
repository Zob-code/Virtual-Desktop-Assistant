...
def respond(voice_data):
    ...
    # 3: greeting
    if there_exists(["how are you","how are you doing","Whats up buddy"]):
    ...
    # date
    if there_exists(["What's today's date","tell me today's date"]):
        strDate = datetime.datetime.now().strftime("%y-%m-%d")    
        engine_speak(f"Sir, the Date is {strDate}")
    # 5: search google
    ...
    if there_exists(["play music"]):
        search_term= voice_data.split("for")[-1]
        url="https://www.jiosaavn.com/"+search_term
        webbrowser.get().open(url)
    ...
    if there_exists(["in wikipedia"]):
        definition=record_audio("what are you looking for")
        ...
        if definitions:
            if definitions[0]:
                engine_speak('im sorry i could not find that, please try a web search')
            elif definitions[1]:
                engine_speak('here is what i found '+definitions[1])
            else:
                engine_speak ('Here is what i found '+definitions[2])
        else:
                engine_speak("im sorry i could not find  for "+definition)


    if there_exists(["exit", "quit", "goodbye"]):
        engine_speak("OK sir SEE YOU soon.,,...,,,,,..,,,,, byee")
        exit()
...
while(1):
    voice_data = record_audio("say something") # get the voice input
    print("Working on it")
    print("Q:", voice_data)
